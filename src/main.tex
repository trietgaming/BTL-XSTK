...
\subsubsection{Quy trình lọc biến từ 34 thuộc tính ban đầu}

Bộ dữ liệu thô chứa 34 thuộc tính, song chỉ một phần nhỏ phù hợp đưa vào mô hình hồi quy giá GPU. Nhóm áp dụng quy trình sàng lọc ba tầng đã trình bày ở Phần~2.2 như sau:

\textbf{Tầng 1 --- Loại theo tỷ lệ dữ liệu khuyết:} Ngưỡng loại bỏ ấn định ở mức \textbf{30\%}. Hình~\ref{fig:missing_ratio} thống kê tỷ lệ thiếu (gồm cả \texttt{NA} và chuỗi rỗng/không hợp lệ) của toàn bộ 34 cột, được vẽ bằng đoạn code sau (chạy trên \texttt{raw\_data} trước mọi bước xử lý):

\begin{lstlisting}[language=R]
# Dem NA va chuoi rong / khong hop le tren toan bo 34 thuoc tinh
...
# Danh dau 8 bien duoc giu lai de to mau khac
...
\begin{figure}[H]
    ...
    \caption{Tỷ lệ dữ liệu khuyết của 34 thuộc tính trong bộ dữ liệu thô ($N = 3.406$). Đường đỏ đứt nét là ngưỡng loại 30\%; cột xanh là 8 biến được giữ lại, cột đỏ bị loại do thiếu, cột xám bị loại vì lý do khác.}
    \label{fig:missing_ratio}
\end{figure}

Ba cột vượt ngưỡng 30\% bị loại trực tiếp: \texttt{DisplayPort\_Connection} ($\sim 75\%$ thiếu), \texttt{Boost\_Clock} ($\sim 57\%$, ngoài ra trùng lặp với \texttt{Core\_Speed}), và \texttt{PSU} ($\sim 35\%$). Riêng \texttt{Release\_Price} dù thiếu $\sim 84\%$ vẫn được giữ vì là biến phụ thuộc Y; các quan sát thiếu Y bị loại bỏ ở Bước~4 thay vì điền khuyết.

\textbf{Tầng 2 --- Loại theo trùng lặp / dẫn xuất:}

\begin{itemize}
  \item \texttt{Memory\_Bandwidth} $=$ \texttt{Memory\_Bus} $\times$ \texttt{Memory\_Speed} / 8 --- thông tin đã được biểu diễn qua \texttt{Memory\_Bus}.
  \item \texttt{Boost\_Clock} có quan hệ tuyến tính chặt với \texttt{Core\_Speed} --- giữ cả hai phá vỡ giả định độc lập.
  \item \texttt{Pixel\_Rate}, \texttt{ROPs}, \texttt{Texture\_Rate}, \texttt{TMUs}, \texttt{L2\_Cache}, \texttt{Best\_Resolution}, \texttt{Resolution\_WxH} mô tả kiến trúc bậc thấp đã được phản ánh gián tiếp qua TDP và xung nhịp lõi.
\end{itemize}

\textbf{Tầng 3 --- Loại theo liên quan kinh tế:} \texttt{Name}, \texttt{Architecture}, \texttt{Process}, \texttt{Direct\_X}, \texttt{Open\_GL}, \texttt{Shader}, \texttt{Power\_Connector}, các cổng kết nối (\texttt{HDMI}, \texttt{VGA}, \texttt{DVI}), và các cờ nhị phân mất cân bằng (\texttt{SLI\_Crossfire}, \texttt{Notebook\_GPU}, \texttt{Dedicated}, \texttt{Integrated}) không có cơ chế tác động trực tiếp đến giá thành. \texttt{Release\_Date} được rút gọn thành \texttt{release\_year} dạng số nguyên.

Sau ba tầng lọc, danh sách rút gọn còn \textbf{8 biến chính thức}.

\subsubsection{Kiểm chứng tương quan giữa biến số và giá GPU}

Để xác nhận các biến được chọn có liên hệ ý nghĩa với biến phụ thuộc, nhóm tính ma trận tương quan Pearson trên tập đã làm sạch (sau điền khuyết, trước chuẩn hóa). Code vẽ heatmap:

\begin{lstlisting}[language=R]
# Tinh ma tran tuong quan Pearson tren tap da lam sach (N = 556)
...
colnames(cor_matrix) <- rownames(cor_matrix) <- display_labels
...
Kết quả tại Hình~\ref{fig:cor_matrix}; bảng số liệu đầy đủ tại Phần~\ref{sec:ket_qua_tuong_quan}.
...
\subsubsection{Bảng tổng hợp 8 biến đã chọn}
...
Kết quả kiểm tra cho thấy một số thuộc tính có lượng lớn giá trị thiếu (\texttt{NA}) như các cổng kết nối (\texttt{DisplayPort}, \texttt{HDMI}, \texttt{VGA}, \texttt{DVI}) và các thông số kiến trúc bậc thấp (\texttt{TMUs}, \texttt{Shader}). Đáng chú ý, mặc dù lệnh \texttt{is.na()} trả về 0 cho nhiều cột, thực tế dữ liệu vẫn còn trống ở dạng chuỗi ký tự, điều này sẽ được xử lý triệt để ở các bước ép kiểu tiếp theo.
...
Kết quả kiểm tra cấu trúc qua lệnh \texttt{str(df)} xác nhận: 3.406 quan sát, 8 biến đã được ép về đúng kiểu (\texttt{numeric} cho các biến số, \texttt{chr} cho biến phân loại, \texttt{int} cho năm phát hành).
...
Số liệu chi tiết tương ứng với heatmap tại Hình~\ref{fig:cor_matrix}: \texttt{tdp} có tương quan mạnh nhất với giá ($r = 0.50$), tiếp theo là \texttt{memory\_size} ($r = 0.34$). Cặp \texttt{core\_speed}--\texttt{release\_year} tương quan khá cao ($r = 0.66$), nhóm đã kiểm định VIF để đánh giá đa cộng tuyến (xem Bảng~\ref{tab:bien}).
