...
Trên tập dữ liệu thuần Consumer/Prosumer ban đầu ($N = 523$), phân phối giá phát hành đã cải thiện đáng kể so với dữ liệu thô. Tuy nhiên, sau khi thực hiện chẩn đoán mô hình sơ bộ (xem Phần~\ref{sec:chan_doan}), nhóm phát hiện thêm 9 quan sát có cấu trúc định giá dị biệt (như card Crossfire kép, card độ tản nhiệt nước, dòng Quadro lọt lưới). Việc loại bỏ 9 quan sát này đưa tập dữ liệu nghiên cứu chính thức về \textbf{$N = 514$}.
...
Mô hình giải thích \textbf{82,78\%} phương sai log-giá (vượt mức $75\%$ trước khi lọc outliers cấu trúc), và \textbf{tất cả 6 biến đều có ý nghĩa} ($p < 0{,}001$). Phương trình hồi quy cuối cùng:
\[
...
\begin{figure}[H]
    ...
    \caption{Histogram phần dư so với đường cong phân phối chuẩn lý tưởng sau khi loại outliers cấu trúc. Cải thiện đột phá với $W = 0{,}989$.}
...
\textbf{Phương sai sai số đồng nhất} (Breusch-Pagan): Hiện tượng phương sai thay đổi gần như bị triệt tiêu triệt để nhờ phép Log-Linear kết hợp với làm sạch Outliers. Chỉ số BP sụt giảm mạnh từ $63{,}43$ xuống vỏn vẹn $\mathbf{38{,}14}$. Đường xu hướng (LOWESS) trên đồ thị phần dư gần như một đường thẳng tuyệt đối.
