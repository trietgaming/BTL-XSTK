...
Tiếp theo, để đánh giá khách quan, nhóm tiến hành kiểm định hai giả định của phương pháp OLS: phương sai sai số đồng nhất (kiểm định Breusch-Pagan) và phần dư có phân phối chuẩn (kiểm định Shapiro-Wilk) trên mô hình tuyến tính cơ sở.

Tuy nhiên, dù mô hình OLS cơ sở có đạt hay không đạt kiểm định, đặc thù của thị trường thiết bị công nghệ là mức giá thường biến động theo \textbf{tỷ lệ phần trăm} (multiplicative) thay vì cộng gộp tuyệt đối (additive). Ví dụ: việc tăng thêm 2GB VRAM sẽ có tác động về giá trên một card trị giá \$1000 hoàn toàn khác so với trên một card trị giá \$200. Do đó, để \textbf{tối ưu hóa mô hình}, nhóm chủ động áp dụng \textbf{phép biến đổi logarit tự nhiên} lên biến phụ thuộc để tạo thành mô hình Log-Linear:
\[
  \log(Y) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_k X_k + \varepsilon
\]

Việc nâng cấp lên mô hình $\log(Y)$ mang lại lợi ích kép: về mặt thống kê, nó giúp nén đuôi phải của dữ liệu (do các mẫu flagship đắt tiền tạo ra), đưa phân phối về dạng đối xứng hơn và xoa dịu các dao động phương sai. Về mặt ý nghĩa thực tiễn, các hệ số trong mô hình Log-Linear rất sắc bén và trực quan: khi một thông số $X_i$ tăng thêm 1 đơn vị (sau chuẩn hóa), giá GPU sẽ thay đổi xấp xỉ $(e^{\beta_i} - 1) \times 100\%$. Mô hình tối ưu này sẽ được sử dụng làm cơ sở để rút ra kết luận cuối cùng.
